---
name: setup-hyperspell
description: >
  Guide the user through integrating Hyperspell into their project. Installs the SDK,
  configures the API key, detects the project type, sets up memory ingestion (OAuth or
  direct), and integrates memory search. Use when a user wants to add Hyperspell to their
  codebase, set up memory search, or connect user accounts via OAuth.
compatibility: Requires Node.js or Python environment with a package manager
metadata:
  author: hyperspell
  version: "1.0"
allowed-tools: Bash Read Write Edit Glob Grep WebFetch(https://docs.hyperspell.com/*) AskUserQuestion
---
...
Fetch and follow the instructions in [detect-project.md](https://docs.hyperspell.com/skills/detect-project.md) to analyze the codebase and determine:
...
Fetch and follow the instructions in [oauth.md](https://docs.hyperspell.com/skills/oauth.md) and implement what applies to this project:
...
- **If the project has a backend:** Fetch and follow the instructions in [direct-ingestion.md](https://docs.hyperspell.com/skills/direct-ingestion.md) to set up memory operations. Use the SDK with your API key and pass the user ID directly.
...
- **If the project has NO backend (frontend-only):** Display this message:
  ...
  ```
  Then follow [direct-ingestion.md](https://docs.hyperspell.com/skills/direct-ingestion.md), noting that the user will need to set up the backend piece separately.
...
**Based on the user's choice, fetch and follow the appropriate section in [search.md](https://docs.hyperspell.com/skills/search.md):**
